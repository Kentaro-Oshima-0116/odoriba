...
- **現行（公開前）**: `index.html` … チーム有効（赤）／個人エントリー無効（グレー）表示
- **公開版（待機中）**: `_index.html` … 個人エントリーを有効化（青 `#2170F7` `.cta--blue` / `.entry-card--personal--live`）。CSS は共通の `assets/css/style.css`（青スタイル仕込み済み）を参照
- **重要**: チーム受付（〜7/18）は個人公開期（7/20〜）には終了しているため、`_index.html` では**チームエントリーも「チーム受付終了」のグレー無効表示**にしている（CTA 4箇所 + カードを `cta--gray cta--disabled` / `.entry-card--closed` 化）。つまり `_index.html` = チーム締切 + 個人エントリーのみ有効
